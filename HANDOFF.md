...
## 2026-08-10 — Blank unassigned duty cells on Service Cards (branch `feature/premium-mobile-roster`, committed locally, not yet pushed)
...
**Also investigated this round**: user reported "Auto Suggest doesn't work" on the Whole Month table. Could not reproduce after extensive testing -- direct function calls, real month navigation (Aug→Sep→Oct, each correctly re-rendering), and a fully synthetic real click-event sequence (pointerdown/mousedown/pointerup/mouseup/click) on the actual button all correctly opened the suggest panel with sensible, correctly month-scoped suggestions (e.g. "Sep 6 · Preacher → Bishop Thomas Low"). Needs more specific detail from the user about what actually happens on their device before a fix can be targeted -- see chat for the exact question asked. One real thing found along the way (not a bug, just a note for future self): the roster script block is a `"use strict"` IIFE with its own private `STATE` object (Index.html ~65808), completely separate from the page's other top-level `STATE` (~15139) -- poking `window.STATE` from outside does nothing to the roster's actual state.
