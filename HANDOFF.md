...
- Compatibility follow-up: Worship Prep was still hard-coded to the separate `lhc-vocal-hero.vercel.app` deployment, which explains why the legacy screen appeared after the initial merge. Both `Index.html` and `dist/index.html` now load local `/practice-game?release=20260720-2`; the route is explicitly `no-store`. Older `game_notes` songs are converted to a melody-guide timeline for the new lanes, and the lane canvas now has a fixed dark fallback plus `ResizeObserver` repainting.
