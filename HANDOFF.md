...
- Added a permanent synchronized backing-track lane directly above the four SATB lanes. It shares the ruler/playhead and visualizes trim, skip, split, loop, speed, and offset settings. The detailed media editor now opens in a dedicated overlay from a compact toolbar action, so it cannot push the backing/SATB timeline out of view.
