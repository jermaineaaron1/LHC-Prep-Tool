...
export function BackingTrackLane({ url, fileName, width, zoom, playhead, settings, onClipsChange, onOpenSettings }: { url: string; fileName: string; width: number; zoom: number; playhead: number | null; settings: BackingTrackSettings; onClipsChange: (clips: BackingTrackClip[]) => void; onOpenSettings: () => void }) {
  ...
  return <div className="flex h-24 border-b border-cyan-300/20 bg-[#061421]">
    <div className="sticky left-0 z-10 flex w-[74px] shrink-0 flex-col justify-center border-r border-cyan-300/20 bg-[#081522] px-2"><b className="text-xs text-cyan-200">TRACK</b><span className="mt-1 truncate text-[9px] text-slate-500">{url ? fileName || 'Backing track' : 'Not loaded'}</span><span className="mt-1 text-[8px] text-cyan-400">Drag · trim · right-click</span></div>
    <div onContextMenu={openMenu} onDoubleClick={handleDoubleClick} onPointerDown={() => { setMenu(null); setSelectedId(null); }} className="relative overflow-hidden" style={{ width }}>
      {url ? <>
        {ordered.map((clip, index) => <div key={clip.id} onPointerDown={event => beginDrag(event, clip, 'move')} onPointerMove={drag} onPointerUp={finishDrag} onPointerCancel={finishDrag} className={`absolute inset-y-3 cursor-grab touch-none rounded-md border bg-cyan-300/10 active:cursor-grabbing ${selectedId === clip.id ? 'z-[4] border-white shadow-[0_0_16px_#67e8f966]' : 'z-[2] border-cyan-300/40'}`} style={{ left: clip.timeline_start * zoom, width: Math.max(12, (clip.source_end - clip.source_start) * zoom), backgroundImage: 'repeating-linear-gradient(90deg,transparent 0,transparent 5px,rgba(103,232,249,.38) 6px,rgba(103,232,249,.38) 8px,transparent 9px,transparent 13px)' }}>
          ...
          <span className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 whitespace-nowrap rounded bg-[#07101d]/90 px-2 py-1 text-[9px] text-cyan-100">{index + 1} · {clip.source_start.toFixed(1)}–{clip.source_end.toFixed(1)}s</span>
      ...
      {menu && <div onPointerDown={event => event.stopPropagation()} className="fixed z-50 w-44 overflow-hidden rounded-lg border border-cyan-300/30 bg-[#08111f] py-1 text-[11px] text-slate-100 shadow-2xl" style={{ left: Math.min(menu.x, window.innerWidth - 190), top: Math.min(menu.y, window.innerHeight - 230) }}>
        {menu.clipId && <><button onClick={() => { const clip = clips.find(item => item.id === menu.clipId); if (clip) splitClip(clip, menu.time); setMenu(null); }} className="block w-full px-3 py-2 text-left hover:bg-white/10">Split here</button><button onClick={() => { const clip = clips.find(item => item.id === menu.clipId); if (clip) clipboardRef.current = { ...clip }; setMenu(null); }} className="block w-full px-3 py-2 text-left hover:bg-white/10">Copy clip</button><button onClick={() => { const clip = clips.find(item => item.id === menu.clipId); if (clip) { clipboardRef.current = { ...clip }; pasteAt(clipEnd(clip)); } }} className="block w-full px-3 py-2 text-left hover:bg-white/10">Duplicate clip</button><button onClick={() => { persist(clips.filter(item => item.id !== menu.clipId)); setSelectedId(null); setMenu(null); }} className="block w-full px-3 py-2 text-left text-rose-200 hover:bg-rose-400/10">Delete clip</button></>}
