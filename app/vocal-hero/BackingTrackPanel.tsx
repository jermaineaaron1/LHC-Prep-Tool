...
export function BackingTrackPanel({ url, kind, fileName, settings, setSettings, uploading, transportTime, transportPlaying, onUpload }: { url: string; kind: 'audio' | 'video'; fileName: string; settings: BackingTrackSettings; setSettings: React.Dispatch<React.SetStateAction<BackingTrackSettings>>; uploading: boolean; transportTime: number | null; transportPlaying: boolean; onUpload: () => void }) {
  ...
  useEffect(() => {
    ...
    if (!url || !media) return;
    if (!transportPlaying && transportOwnedRef.current) { media.pause(); transportOwnedRef.current = false; }
    if (transportTime === null) return;
    const target = Math.max(settings.trim_start, settings.trim_start + transportTime + settings.timeline_offset);
    if (Math.abs(media.currentTime - target) > .16) media.currentTime = target;
    media.playbackRate = settings.speed;
    if (transportPlaying) { transportOwnedRef.current = true; configureGraph(); if (media.paused) void media.play(); }
  }, [url, transportTime, transportPlaying, settings.trim_start, settings.timeline_offset, settings.speed]);
  ...
  return <div className="mt-3 max-h-[260px] overflow-y-auto rounded-xl border border-cyan-300/20 bg-black/20 p-3">
    ...
    {url && <div className="mt-3 flex flex-wrap items-center gap-3 rounded-lg border border-fuchsia-300/20 bg-fuchsia-500/[.04] px-3 py-2 text-[11px]"><label className="text-slate-300">Timeline offset <input type="number" step="0.01" value={settings.timeline_offset} onChange={event => update({ timeline_offset: Number(event.target.value) })} className="ml-2 w-20 rounded border border-white/10 bg-[#050816] px-2 py-1 text-white" /> s</label><span className="text-slate-400">SATB transport {transportPlaying ? 'is driving this track' : 'will drive this track on Play'} · media starts at trim start + timeline position + offset.</span></div>}
  </div>;
}
