...
export function BackingTrackPanel({ url, kind, fileName, settings, setSettings, uploading, transportTime, transportPlaying, onUpload }: { url: string; kind: 'audio' | 'video'; fileName: string; settings: BackingTrackSettings; setSettings: React.Dispatch<React.SetStateAction<BackingTrackSettings>>; uploading: boolean; transportTime: number | null; transportPlaying: boolean; onUpload: () => void }) {
  ...
  return <div className="rounded-xl border border-cyan-300/20 bg-black/20 p-3">
    ...
    {url && <div className="mt-3 flex flex-wrap items-center gap-3 rounded-lg border border-fuchsia-300/20 bg-fuchsia-500/[.04] px-3 py-2 text-[11px]"><label className="text-slate-300">Track starts on timeline <input type="number" step="0.01" value={settings.timeline_offset} onChange={event => update({ timeline_offset: Number(event.target.value) })} className="ml-2 w-20 rounded border border-white/10 bg-[#050816] px-2 py-1 text-white" /> s</label><span className="text-slate-400">Set a positive value to begin the backing track later, or drag the cyan track lane directly against the SATB notes.</span></div>}
  </div>;
}
