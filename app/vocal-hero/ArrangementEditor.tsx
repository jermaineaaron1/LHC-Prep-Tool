...
export function ArrangementEditor({ song, onClose, onSave }: { song: Song; onClose: () => void; onSave: (values: EditableSong) => Promise<void>; }) {
  ...
  const [rangePart, setRangePart] = useState<number | null>(null);
  ...
  function clearPlaybackSelections() { setPlayScope('all'); setPlayParts([true, true, true, true]); setPlayRange({ start: 0, end: 8 }); setRangePart(null); setSelectedId(null); setSelectedIds([]); }
  function selectPlayPart(part: number, additive = false) { setPlayParts(current => additive ? current.map((enabled, index) => index === part ? !enabled : enabled) : VOICES.map((_, index) => index === part)); setPlayScope('all'); setRangePart(null); setSelectedId(null); setSelectedIds([]); }
  function updateRangeSelection(part: number, range: { start: number; end: number }) { setPlayRange(range); setRangePart(part); const ids = notes.filter(note => (note.part === part || (note.part === -1 && part === selectedPart)) && note.end >= range.start && note.start <= range.end).map(note => note.id); setSelectedIds(ids); setSelectedId(ids[0] ?? null); }
  function selectRange(part: number) { setPlayScope('range'); setPlayParts(VOICES.map((_, index) => index === part)); setRangePart(part); setSelectedId(null); }
