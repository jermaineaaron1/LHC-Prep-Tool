...
export function ArrangementEditor({ song, onClose, onSave }: { song: Song; onClose: () => void; onSave: (values: EditableSong) => Promise<void>; }) {
  ...
  function selectNote(id: string, rangeSelect = false) { const note = notes.find(item => item.id === id); if (!note) return; if (rangeSelect && selected) { setPlayRange({ start: Math.min(selected.start, note.start), end: Math.max(selected.end, note.end) }); setPlayScope('range'); setPlayParts([true, true, true, true]); setSelectedId(null); return; } setSelectedPart(note.part < 0 ? 0 : note.part); setSelectedId(id); setPlayScope('note'); }
  ...
  function clearPlaybackSelections() { setPlayScope('all'); setPlayParts([true, true, true, true]); setPlayRange({ start: 0, end: 8 }); setSelectedId(null); }
  function selectPlayPart(part: number, additive = false) { setPlayParts(current => additive ? current.map((enabled, index) => index === part ? !enabled : enabled) : VOICES.map((_, index) => index === part)); setPlayScope('all'); setSelectedId(null); }
  ...
  return <div className="fixed inset-0 z-50 overflow-hidden bg-[#020510] text-slate-100">
    ...
    <div className="flex h-[calc(100vh-64px)] min-h-[620px] overflow-hidden">
      ...
      <main className="min-w-0 flex-1 overflow-hidden bg-[radial-gradient(circle_at_50%_0%,#28135055,transparent_30%),#080b1c]">
        <EditorToolbar tool={tool} setTool={setTool} playScope={playScope} playParts={playParts} onPlayAll={clearPlaybackSelections} onPlayPart={selectPlayPart} playRange={playRange} playhead={playhead} onClearSelection={clearPlaybackSelections} zoom={zoom} setZoom={setZoom} onDuplicate={duplicateSelected} onPlay={previewArrangement} isPlaying={isPlaying} onRecord={() => void toggleRecording()} recording={recording} onPlayTake={playRecordedTake} hasTake={Boolean(recordingUrl)} onSave={() => void save()} saving={saving} />
...
function EditorToolbar({ tool, setTool, playScope, playParts, onPlayAll, onPlayPart, playRange, playhead, onClearSelection, zoom, setZoom, onDuplicate, onPlay, isPlaying, onRecord, recording, onPlayTake, hasTake, onSave, saving }: { tool: EditorTool; setTool: (tool: EditorTool) => void; playScope: PlaybackScope; playParts: boolean[]; onPlayAll: () => void; onPlayPart: (part: number, additive?: boolean) => void; playRange: { start: number; end: number }; playhead: number | null; onClearSelection: () => void; zoom: number; setZoom: (value: number) => void; onDuplicate: () => void; onPlay: () => void; isPlaying: boolean; onRecord: () => void; recording: boolean; onPlayTake: () => void; hasTake: boolean; onSave: () => void; saving: boolean }) {
  ...
  const status = playScope === 'range' ? `Range ${playRange.start.toFixed(2)}s–${playRange.end.toFixed(2)}s` : playScope === 'note' ? 'Selected note' : playParts.every(Boolean) ? 'All voices from start' : `${VOICES.filter((_, index) => playParts[index]).join(' + ')} from start`;
...
function PianoTrack({ name, part, notes, selectedId, tool, playhead, selectedRange, width, zoom, onAdd, onSelect, onRemove, onResize, onRangeChange, onRangeEnd, onEmptyClick }: { name: string; part: number; notes: SongNote[]; selectedId: string | null; tool: EditorTool; playhead: number | null; selectedRange: { start: number; end: number } | null; width: number; zoom: number; onAdd: (part: number, event: React.MouseEvent<HTMLDivElement>) => void; onSelect: (id: string, rangeSelect?: boolean) => void; onRemove: (id: string) => void; onResize: (id: string, end: number) => void; onRangeChange: (range: { start: number; end: number }) => void; onRangeEnd: () => void; onEmptyClick: () => void }) {
