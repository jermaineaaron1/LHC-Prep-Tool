...
function EditorToolbar({ tool, setTool, playScope, setPlayScope, playParts, onTogglePart, onToggleAllParts, playRange, setPlayRange, playhead, onClearRange, zoom, setZoom, onDuplicate, onPlay, isPlaying, onRecord, recording, onPlayTake, hasTake, onSave, saving }: { tool: EditorTool; setTool: (tool: EditorTool) => void; playScope: PlaybackScope; setPlayScope: (scope: PlaybackScope) => void; playParts: boolean[]; onTogglePart: (part: number) => void; onToggleAllParts: () => void; playRange: { start: number; end: number }; setPlayRange: (range: { start: number; end: number }) => void; playhead: number | null; onClearRange: () => void; zoom: number; setZoom: (value: number) => void; onDuplicate: () => void; onPlay: () => void; isPlaying: boolean; onRecord: () => void; recording: boolean; onPlayTake: () => void; hasTake: boolean; onSave: () => void; saving: boolean }) {
...
function PianoTrack({ name, part, notes, selectedId, tool, playhead, selectedRange, width, zoom, onAdd, onSelect, onRemove, onResize, onRangeChange, onRangeEnd }: { name: string; part: number; notes: SongNote[]; selectedId: string | null; tool: EditorTool; playhead: number | null; selectedRange: { start: number; end: number } | null; width: number; zoom: number; onAdd: (part: number, event: React.MouseEvent<HTMLDivElement>) => void; onSelect: (id: string, rangeSelect?: boolean) => void; onRemove: (id: string) => void; onResize: (id: string, end: number) => void; onRangeChange: (range: { start: number; end: number }) => void; onRangeEnd: () => void }) {
  ...
  function endRange(event: React.PointerEvent<HTMLDivElement>) { const active = selecting.current; if (!active) return; const end = timeAt(event); if (Math.abs(end - active.start) >= .1) onRangeEnd(); if (event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId); selecting.current = null; }
