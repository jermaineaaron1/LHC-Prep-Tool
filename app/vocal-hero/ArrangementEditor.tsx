...
type BarRhythmSuggestion = { bar: MusicalBar; cursor: number; remainingBeats: number; remainingSeconds: number; primary: NoteValueDefinition; alternatives: NoteValueDefinition[]; exact: boolean };
...
function barRhythmSuggestionAt(bars: MusicalBar[], time: number): BarRhythmSuggestion | null {
  const safe = Math.max(0, time);
  let bar = bars.find(item => safe >= item.start - .0005 && safe < item.end - .0005) ?? bars.at(-1);
  if (!bar) return null;
  let cursor = Math.max(bar.start, safe);
  if (cursor >= bar.end - .0005) {
    bar = bars.find(item => item.number === bar!.number + 1) ?? bar;
    cursor = bar.start;
  }
  const remainingSeconds = Math.max(.001, bar.end - cursor);
  const remainingBeats = durationInBeats(bars, cursor, bar.end);
  const ranked = NOTE_VALUES
    .map(item => ({ item, duration: noteDurationAt(bars, cursor, item.value) }))
    .filter(candidate => candidate.duration <= remainingSeconds + .001)
    .sort((a, b) => Math.abs(remainingSeconds - a.duration) - Math.abs(remainingSeconds - b.duration) || b.duration - a.duration);
  const primary = ranked[0]?.item ?? NOTE_VALUES.at(-1)!;
  return {
    bar,
    cursor,
    remainingBeats,
    remainingSeconds,
    primary,
    alternatives: ranked.slice(0, 4).map(candidate => candidate.item),
    exact: Math.abs(noteDurationAt(bars, cursor, primary.value) - remainingSeconds) <= .002,
  };
}
...
export function ArrangementEditor({ song, onClose, onSave }: { song: Song; onClose: () => void; onSave: (values: EditableSong) => Promise<void>; }) {
  ...
  const drawRhythmSuggestion = useMemo(() => barRhythmSuggestionAt(musicalBars, selected?.end ?? playhead ?? 0), [musicalBars, selected, playhead]);
  ...
  function addNote(part = selectedPart, start = notes.reduce((latest, note) => Math.max(latest, note.end), 0), midi = 60, end?: number) {
    ...
    let candidate = quantizeNote({ id, part, midi, start, end: end ?? start + noteDurationAt(musicalBars, start, value), lyric: 'New lyric', velocity: 100 }, musicalBars, division);
    const containingBar = musicalBars.find(item => candidate.start >= item.start - .0005 && candidate.start < item.end - .0005);
    const crossesBar = end === undefined && Boolean(containingBar) && candidate.end > containingBar!.end + .0005;
    if (crossesBar && containingBar) candidate = { ...candidate, end: containingBar.end };
    ...
    if (crossesBar && containingBar) {
      const fitted = closestNoteValue(musicalBars, candidate.start, candidate.end).item;
      setEditorNotice(`${noteValue(value).short} would cross Bar ${containingBar.number}, so it was fitted to the remaining ${durationInBeats(musicalBars, candidate.start, candidate.end).toFixed(2)} beats (${fitted.short}). You may still choose or resize any rhythmic value.`);
    } else setEditorNotice(null);
  }
...
function DrawNoteValuePicker({ value, onChange, suggestion }: { value: RhythmicNoteValue; onChange: (value: RhythmicNoteValue) => void; suggestion?: BarRhythmSuggestion | null }) {
  ...
  return <div className="flex shrink-0 items-center rounded-xl border border-fuchsia-300/35 bg-[linear-gradient(135deg,#241044,#0b1831)] p-1 shadow-[0_0_22px_#d946ef22]" aria-label="Draw note length">
    ...
    {suggestion && <button type="button" onClick={() => onChange(suggestion.primary.value)} title={`${suggestion.exact ? 'Exactly fills' : 'Fits within'} the ${suggestion.remainingBeats.toFixed(2)} beats remaining in Bar ${suggestion.bar.number}. This is a suggestion, not an automatic override.`} className="ml-1 flex h-8 items-center gap-1.5 rounded-lg border border-cyan-300/25 bg-cyan-300/[.08] px-2 text-left hover:border-cyan-200/55"><span className="font-['Segoe_UI_Symbol','Noto_Music',serif] text-lg leading-none text-cyan-200">{suggestion.primary.symbol}</span><span><small className="block text-[7px] font-bold uppercase tracking-[.13em] text-cyan-300">Suggested</small><b className="block whitespace-nowrap text-[9px] text-white">{suggestion.primary.short}</b></span></button>}
    {open && rect && createPortal(<div ref={panelRef} role="dialog" aria-label="Rhythmic note glossary" className="fixed z-[100] overflow-y-auto rounded-2xl border border-fuchsia-300/35 bg-[#080b1d]/[.98] p-4 shadow-[0_24px_80px_#000c,0_0_45px_#a855f733] backdrop-blur-xl" style={{ left: panelLeft, top: panelTop, width: panelWidth, maxHeight: `calc(100vh - ${panelTop + 12}px)` }}>
      ...
      {suggestion && <section className="mb-3 flex flex-wrap items-center gap-2 rounded-xl border border-cyan-300/25 bg-[linear-gradient(90deg,#082138,#211044)] p-3"><span className="mr-auto"><small className="block text-[8px] font-bold uppercase tracking-[.16em] text-cyan-300">Bar {suggestion.bar.number} rhythm assistant</small><b className="text-xs text-white">{suggestion.remainingBeats.toFixed(2)} beats remain · {suggestion.exact ? 'exact-fit value available' : 'longest fitting value suggested'}</b></span>{suggestion.alternatives.map((item, index) => <button key={item.value} type="button" onClick={() => { onChange(item.value); setOpen(false); }} className={`rounded-lg border px-2.5 py-1.5 text-[10px] ${index === 0 ? 'border-cyan-200/50 bg-cyan-300/15 text-cyan-100' : 'border-white/10 text-slate-300 hover:border-fuchsia-300/35'}`}><span className="mr-1 font-['Segoe_UI_Symbol','Noto_Music',serif] text-base">{item.symbol}</span>{item.short}</button>)}</section>}
...
function EditorToolbar({ tool, setTool, drawNoteValue, onDrawNoteValueChange, drawSuggestion, playScope, playParts, onPlayAll, onPlayPart, playRange, playhead, onClearSelection, selectedCount, onRemove, canUndo, canRedo, onUndo, onRedo, zoom, setZoom, onDuplicate, onCopy, onPaste, clipboardCount, onPlay, onPlayFromStart, onPause, onStop, onSkip, isPlaying, isPaused, onRecord, recording, onPlayTake, hasTake, onSave, saving }: { tool: EditorTool; setTool: (tool: EditorTool) => void; drawNoteValue: RhythmicNoteValue; onDrawNoteValueChange: (value: RhythmicNoteValue) => void; drawSuggestion?: BarRhythmSuggestion | null; playScope: PlaybackScope; playParts: boolean[]; onPlayAll: () => void; onPlayPart: (part: number, additive?: boolean) => void; playRange: { start: number; end: number }; playhead: number | null; onClearSelection: () => void; selectedCount: number; onRemove: () => void; canUndo: boolean; canRedo: boolean; onUndo: () => void; onRedo: () => void; zoom: number; setZoom: (value: number) => void; onDuplicate: () => void; onCopy: () => void; onPaste: () => void; clipboardCount: number; onPlay: () => void; onPlayFromStart: () => void; onPause: () => void; onStop: () => void; onSkip: (seconds: number) => void; isPlaying: boolean; isPaused: boolean; onRecord: () => void; recording: boolean; onPlayTake: () => void; hasTake: boolean; onSave: () => void; saving: boolean }) {
  ...
  return <div className="border-b border-white/10 bg-[#0a0c20] text-xs">
    ...
    <div className="flex h-14 items-center gap-2 overflow-x-auto border-t border-white/[.06] px-3">
      ...
      {tool === 'draw' && <DrawNoteValuePicker value={drawNoteValue} onChange={onDrawNoteValueChange} suggestion={drawSuggestion} />}
