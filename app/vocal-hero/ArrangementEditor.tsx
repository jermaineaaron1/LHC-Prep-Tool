...
export function ArrangementEditor({ song, onClose, onSave }: { song: Song; onClose: () => void; onSave: (values: EditableSong) => Promise<void>; }) {
  ...
  function previewArrangement() {
    ...
    const preview = ordered.filter(note => note.start < first + 20);
    const last = Math.min(20, Math.max(...preview.map(note => note.end - first)));
    ...
    preview.forEach(note => {
      ...
      gain.gain.setValueAtTime(Math.max(.015, Math.min(.08, note.velocity / 1600)), context.currentTime + at);
      oscillator.connect(gain).connect(context.destination);
      oscillator.start(context.currentTime + at);
      oscillator.stop(context.currentTime + at + length);
    });
  ...
  return <div className="fixed inset-0 z-50 overflow-hidden bg-[#020510] text-slate-100">
    ...
    <div className="flex h-[calc(100vh-64px)] min-h-[620px] overflow-hidden">
      ...
      <main className="min-w-0 flex-1 overflow-hidden bg-[radial-gradient(circle_at_50%_0%,#28135055,transparent_30%),#080b1c]">
        <EditorToolbar tool={tool} setTool={setTool} zoom={zoom} setZoom={setZoom} onDuplicate={duplicateSelected} onPlay={previewArrangement} isPlaying={isPlaying} onRecord={() => void toggleRecording()} recording={recording} onPlayTake={playRecordedTake} hasTake={Boolean(recordingUrl)} onSave={() => void save()} saving={saving} />
...
function EditorToolbar({ tool, setTool, zoom, setZoom, onDuplicate, onPlay, isPlaying, onRecord, recording, onPlayTake, hasTake, onSave, saving }: { tool: EditorTool; setTool: (tool: EditorTool) => void; zoom: number; setZoom: (value: number) => void; onDuplicate: () => void; onPlay: () => void; isPlaying: boolean; onRecord: () => void; recording: boolean; onPlayTake: () => void; hasTake: boolean; onSave: () => void; saving: boolean }) {
