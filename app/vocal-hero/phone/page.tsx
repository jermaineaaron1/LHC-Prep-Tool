...
function PhoneGame() {
  ...
  if (!session) return <Join room={room} setRoom={setRoom} name={name} setName={setName} part={partIndex} setPart={setPartIndex} error={error} join={join} mic={mic} level={level} diag={diag} audioBlocked={audioBlocked} testMic={testMic} onResume={() => { void pitchRef.current?.resume().then(() => setAudioBlocked(Boolean(pitchRef.current?.isSuspended))); }} />;
