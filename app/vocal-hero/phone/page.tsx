...
function PhoneGame() {
  ...
  return <PhoneLive song={song!} notes={notes} transpose={transpose} guide={isGuideMelody(notes)} part={partIndex} elapsed={timeline.songElapsed} pitch={pitch} score={score} hits={hits} sections={sections} mic={mic} fullBoard={fullBoard} setFullBoard={setFullBoard} trail={trailRef.current} />;
}
...
function PhoneLobby({ song, code, part, players, player, mic, testMic, ready, error, difficulty, setDifficulty, latencySec, applyLatency, transpose, setTranspose, hasBackingTrack }: { song: Song; code: string; part: number; players: SessionPlayer[]; player: SessionPlayer | null; mic: string; testMic: () => void; ready: () => void; error: string; difficulty: Difficulty; setDifficulty: (next: Difficulty) => void; latencySec: number; applyLatency: (seconds: number) => void; transpose: number; setTranspose: (next: number) => void; hasBackingTrack: boolean }) { return <PhoneShell><div className="mt-5 text-center"><p className="text-sm text-slate-300">{song.title}</p><p className="mt-1 text-xs text-slate-500">{song.artist || 'Vocal Hero arrangement'}</p><p className="mt-4 text-xs text-slate-400">ROOM CODE</p><p className="font-mono text-3xl font-bold tracking-[.25em] text-[#ffd15c]">{code}</p></div><p className="mt-7 text-center text-xs tracking-[.18em] text-slate-400">CHOOSE YOUR PART</p><div className="mt-3 grid grid-cols-2 gap-2">{VOICES.map((voice, index) => <div key={voice} className="rounded-xl border p-3" style={{ borderColor: index === part ? COLOURS[index] : '#ffffff16', background: index === part ? `${COLOURS[index]}14` : '#07111d' }}><b style={{ color: COLOURS[index] }}>{voice[0]} <span className="ml-1 text-sm">{voice}</span></b><p className="mt-2 text-xs text-slate-400">{players.filter(item => item.part_index === index).length} players</p></div>)}</div><div className="mt-5"><DifficultyPicker value={difficulty} onChange={setDifficulty} colour={COLOURS[part]} /></div><div className="mt-5"><LatencyRow colour={COLOURS[part]} latencySec={latencySec} onChange={applyLatency} /></div><div className="mt-5"><TransposePicker value={transpose} onChange={setTranspose} colour={COLOURS[part]} hasBackingTrack={hasBackingTrack} /></div><div className="mt-5 rounded-2xl border border-white/10 bg-black/20 p-4"><p className="text-xs text-slate-400">MIC CHECK</p><button onClick={testMic} className={`mt-2 text-sm ${mic === 'blocked' ? 'text-rose-300' : 'text-emerald-300'}`}>● {mic === 'ready' ? 'Microphone detected' : mic === 'blocked' ? 'Microphone blocked — tap to retry' : mic === 'checking' ? 'Checking…' : 'Test microphone'}</button></div>{error && <p className="mt-3 rounded-xl border border-rose-400/30 bg-rose-950/40 px-3 py-2 text-xs text-rose-200">{error}</p>}<button onClick={ready} className="vh-start-button mt-5 w-full">✓ {player?.ready_at ? 'I’M READY' : 'TAP WHEN READY'}</button><p className="mt-2 text-center text-[10px] text-slate-500">Tapping ready asks for the microphone now, so the permission prompt cannot land on the downbeat.</p></PhoneShell>; }
...
function PhoneLive({ song, notes, transpose, guide, part, elapsed, pitch, score, hits, sections, mic, fullBoard, setFullBoard, trail }: { song: Song; notes: SongNote[]; transpose: number; guide: boolean; part: number; elapsed: number; pitch: number; score: number; hits: Record<string, boolean>; sections: SectionScore[]; mic: string; fullBoard: boolean; setFullBoard: (value: boolean) => void; trail: TrailSample[] }) {
