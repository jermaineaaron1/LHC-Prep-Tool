...
function PhoneGame() {
  ...
  useEffect(() => { if (session?.status !== 'ended' || !player || !scoreRef.current) return; pitchRef.current?.stop(); setReview(summariseRound(resultsRef.current, notes)); const stats = scoreRef.current.stats; void scoreRef.current.stop().then(() => warmUpRef.current ? undefined : savePlayerRoundStats({ session_id: session.id, player_id: player.id, score, accuracy: stats.accuracy, notes_attempted: stats.attempted, notes_hit: stats.hit })); }, [player, score, session?.status]);
...
function PhoneLive({ song, notes, transpose, warmUp, guide, part, elapsed, pitch, score, hits, sections, mic, fullBoard, setFullBoard, trail }: { song: Song; notes: SongNote[]; transpose: number; warmUp: boolean; guide: boolean; part: number; elapsed: number; pitch: number; score: number; hits: Record<string, boolean>; sections: SectionScore[]; mic: string; fullBoard: boolean; setFullBoard: (value: boolean) => void; trail: TrailSample[] }) {
