...
function PhoneGame() {
  ...
  // Which failure, not merely that there was one. Every microphone problem used
  // to end up as the single word "blocked", which is actively misleading in an
  // installed app: the commonest cause there is a permission that was never
  // ASKED for on this scope, rather than one that was refused.
  const [micReason, setMicReason] = useState<MicFailure | null>(null);
  const [micEnv, setMicEnv] = useState<MicEnv>({ secure: true, hasMediaDevices: true, standalone: false, permission: 'unknown' });
  const readEnv = useCallback(() => { void PitchEngine.environment().then(setMicEnv); }, []);
  useEffect(() => { readEnv(); }, [readEnv]);
  ...
  // the engine.
  const [diag, setDiag] = useState({ hz: 0, level: 0, confidence: 0, rate: 0, dc: 0, capture: '—' }); const [score, setScore] = useState(0); const [hits, setHits] = useState<Record<string, boolean>>({}); const [fullBoard, setFullBoard] = useState(false); const [pausedElapsed, setPausedElapsed] = useState(0);
  ...
  async function testMic() { setMic('checking'); await startPitchTracking(); if (!player) return; await updatePlayerLobbyState(player.id, { ready_at: player.ready_at ?? null, mic_status: pitchRef.current?.isRunning ? 'ready' : 'blocked' }); }
  ...
  if (!session) return <Join room={room} setRoom={setRoom} name={name} setName={setName} part={partIndex} setPart={setPartIndex} error={error} join={join} mic={mic} level={level} diag={diag} audioBlocked={audioBlocked} micReason={micReason} micEnv={micEnv} testMic={testMic} onResume={() => { void pitchRef.current?.resume().then(() => setAudioBlocked(Boolean(pitchRef.current?.isSuspended))); }} />;
  if (session.status === 'lobby') return <PhoneLobby song={song!} code={session.room_code} part={partIndex} players={players} player={player} mic={mic} testMic={testMic} ready={readyUp} error={error} difficulty={difficulty} setDifficulty={setDifficulty} latencySec={latencySec} applyLatency={applyLatency} transpose={transpose} setTranspose={setTranspose} hasBackingTrack={!!(song?.audio_url || song?.backing_media_url)} warmUp={warmUp} setWarmUp={setWarmUp} level={level} diag={diag} audioBlocked={audioBlocked} micReason={micReason} micEnv={micEnv} getEngine={() => pitchRef.current} onResume={() => { void pitchRef.current?.resume().then(() => setAudioBlocked(Boolean(pitchRef.current?.isSuspended))); }} />;
 ...
 */
function PhoneShell({ children, fill = false }: { children: React.ReactNode; fill?: boolean }) {
  return <main
    className={`vh-app text-slate-100${fill ? ' flex h-[100dvh] flex-col overflow-hidden' : ''}`}
    style={{
  ...
  >
    <header className={`flex items-center justify-between border-b border-white/10${fill ? ' shrink-0 pb-2' : ' pb-4'}`}><span className="text-slate-400">☰</span><PhoneBrand /><span className="text-slate-400">?</span></header>
    {fill ? <div className="flex min-h-0 flex-1 flex-col overflow-y-auto">{children}</div> : children}
  </main>;
}
type MicEnv = { secure: boolean; hasMediaDevices: boolean; standalone: boolean; permission: string };

/** What went wrong, in words that name the fix.
 *
 *  An installed PWA keeps its OWN microphone permission, separate from the
 *  browser that installed it. A singer who allowed the microphone in Chrome and
 *  then opened the app from their home screen has granted nothing, and will
 *  reasonably swear the permission is already on -- so when this is running as
 *  an installed app the wording says so, and offers the browser as a way past
 *  it rather than sending them round a settings screen that already reads
 *  Allow. */
function MicHelp({ reason, env }: { reason: MicFailure | null; env: MicEnv }) {
  const openInBrowser = () => { window.open(window.location.href, '_blank', 'noopener'); };
  if (!reason) {
    // Nothing has failed yet, but a denial already on record means the next
    // attempt will fail with no prompt at all, so it is worth saying up front.
    if (env.permission !== 'denied') return null;
    return <p className="mt-2 rounded-xl border border-rose-400/30 bg-rose-950/40 px-3 py-2 text-[11px] text-rose-200">This {env.standalone ? 'installed app' : 'browser'} has the microphone set to <b>block</b>. Open the padlock or menu to Permissions, set Microphone to Allow, then reload.</p>;
  }
  const body: Record<MicFailure, React.ReactNode> = {
    blocked: env.standalone
      ? <>The installed app was refused the microphone. It keeps a <b>separate</b> permission from your browser, so allowing it in Chrome does not cover this window. Allow it here, or open the app in the browser instead.</>
      : <>Permission was refused. Tap the padlock in the address bar, then Permissions, then Microphone, then Allow, and reload.</>,
    insecure: <>This page is not on HTTPS, and browsers only hand out microphones over a secure connection.</>,
    unsupported: <>This window offers no microphone API at all{env.standalone ? ', which some installed app shells are known to do' : ''}. Open the app in your browser and it will work.</>,
    notfound: <>No microphone was offered by this device. If a headset is plugged in, try removing it.</>,
    busy: <>Another app is holding the microphone. Close any call, recorder or voice assistant, then try again.</>,
    unknown: <>The microphone could not be started. Reload the page and try once more.</>,
  };
  return <div className="mt-2 rounded-xl border border-amber-400/30 bg-amber-950/30 px-3 py-2 text-[11px] text-amber-100">
    <p>{body[reason]}</p>
    {env.standalone && (reason === 'blocked' || reason === 'unsupported') &&
      <button type="button" onClick={openInBrowser} className="mt-2 w-full rounded-lg border border-amber-300/40 px-3 py-2 text-[11px] font-semibold">Open in the browser instead</button>}
  </div>;
}

function Join({ room, setRoom, name, setName, part, setPart, error, join, mic, level, diag, audioBlocked, micReason, micEnv, testMic, onResume }: { room: string; setRoom: (v: string) => void; name: string; setName: (v: string) => void; part: number; setPart: (v: number) => void; error: string; join: (event: React.FormEvent) => void; mic: string; level: number; diag: { hz: number; level: number; confidence: number; rate: number; dc: number; capture: string }; audioBlocked: boolean; micReason: MicFailure | null; micEnv: MicEnv; testMic: () => void; onResume: () => void }) { return <PhoneShell><form onSubmit={join} className="mx-auto mt-10 max-w-md vh-panel p-5"><p className="text-center text-xs tracking-[.2em] text-fuchsia-300">JOIN LOBBY</p><h1 className="mt-3 text-center text-3xl font-black">Sing your part.</h1><label className="mt-6 block text-xs text-slate-400">Room code<input value={room} onChange={e => setRoom(e.target.value.toUpperCase())} maxLength={5} className="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-center font-mono text-2xl tracking-[.25em]" /></label><label className="mt-4 block text-xs text-slate-400">Nickname<input value={name} onChange={e => setName(e.target.value)} className="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-base" /></label><p className="mt-5 text-xs tracking-[.15em] text-slate-400">CHOOSE YOUR PART</p><div className="mt-2 grid grid-cols-2 gap-2">{VOICES.map((voice, index) => <button type="button" key={voice} onClick={() => setPart(index)} className="rounded-xl border p-3 text-left" style={{ borderColor: part === index ? COLOURS[index] : '#ffffff18', background: part === index ? `${COLOURS[index]}18` : '#07111d' }}><b className="text-2xl" style={{ color: COLOURS[index] }}>{voice[0]}</b><span className="ml-2 text-sm">{voice}</span></button>)}</div>{error && <p className="mt-4 text-sm text-rose-300">{error}</p>}<div className="mt-5 rounded-2xl border border-white/10 bg-black/20 p-4"><div className="flex items-center justify-between"><p className="text-xs text-slate-400">MICROPHONE</p><button type="button" onClick={testMic} className="-my-3 -mr-3 px-3 py-3 text-xs text-emerald-300">{mic === 'ready' ? 'restart' : mic === 'checking' ? 'checking…' : 'test now'}</button></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-white/10"><span className="block h-full rounded-full bg-emerald-400 transition-[width] duration-75" style={{ width: `${Math.min(100, Math.round(level * 400))}%` }} /></div><p className="mt-1 text-[10px] text-slate-500">{mic === 'unknown' ? 'Test it here before joining — no room needed.' : level > 0.002 ? 'Sound is reaching the app.' : 'Say something. If the bar stays flat, no audio is arriving.'}</p><div className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1 border-t border-white/10 pt-2 font-mono text-[10px] text-slate-500"><span>note</span><b className={diag.hz > 0 ? 'text-emerald-300' : 'text-slate-600'}>{diag.hz > 0 ? PitchEngine.toNoteName(diag.hz) + '  ' + diag.hz.toFixed(1) + ' Hz' : '—'}</b><span>level</span><b className={diag.level > 0.002 ? 'text-emerald-300' : 'text-slate-600'}>{diag.level.toFixed(4)}</b><span>confidence</span><b>{diag.confidence.toFixed(2)}</b><span>sample rate</span><b>{diag.rate ? (diag.rate / 1000).toFixed(1) + ' kHz' : '—'}</b><span>dc offset</span><b className={Math.abs(diag.dc) > 0.05 ? 'text-rose-300' : 'text-slate-600'}>{diag.dc.toFixed(3)}{Math.abs(diag.dc) > 0.05 ? ' CORRECTED' : ''}</b><span>capture</span><b>{diag.capture}</b><span>build</span><b>{ENGINE_BUILD}</b><span>audio</span><b className={audioBlocked ? 'text-amber-300' : mic === 'ready' ? 'text-emerald-300' : ''}>{audioBlocked ? 'PAUSED by browser' : mic === 'ready' ? 'running' : mic === 'blocked' ? 'BLOCKED' : 'not started'}</b><span>window</span><b>{micEnv.standalone ? 'installed app' : 'browser tab'}</b><span>permission</span><b className={micEnv.permission === 'granted' ? 'text-emerald-300' : micEnv.permission === 'denied' ? 'text-rose-300' : 'text-slate-500'}>{micEnv.permission}</b></div><MicHelp reason={micReason} env={micEnv} /><MicReportButton getEngine={() => null} context="phone-join" />{audioBlocked && <button type="button" onClick={onResume} className="mt-2 w-full rounded-xl border border-amber-400/40 bg-amber-950/40 px-3 py-2 text-xs text-amber-100">Audio is paused by the browser — tap to switch it on.</button>}</div><button className="vh-start-button mt-6 w-full">JOIN LOBBY</button></form></PhoneShell>; }
function PhoneLobby({ song, code, part, players, player, mic, testMic, ready, error, difficulty, setDifficulty, latencySec, applyLatency, transpose, setTranspose, hasBackingTrack, warmUp, setWarmUp, level, audioBlocked, micReason, micEnv, onResume, diag, getEngine }: { song: Song; code: string; part: number; players: SessionPlayer[]; player: SessionPlayer | null; mic: string; testMic: () => void; ready: () => void; error: string; difficulty: Difficulty; setDifficulty: (next: Difficulty) => void; latencySec: number; applyLatency: (seconds: number) => void; transpose: number; setTranspose: (next: number) => void; hasBackingTrack: boolean; warmUp: boolean; setWarmUp: (next: boolean) => void; level: number; audioBlocked: boolean; micReason: MicFailure | null; micEnv: MicEnv; onResume: () => void; getEngine: () => PitchEngine | null; diag: { hz: number; level: number; confidence: number; rate: number; dc: number; capture: string } }) { return <PhoneShell><div className="mt-5 text-center"><p className="text-sm text-slate-300">{song.title}</p><p className="mt-1 text-xs text-slate-500">{song.artist || 'Vocal Hero arrangement'}</p><p className="mt-4 text-xs text-slate-400">ROOM CODE</p><p className="font-mono text-3xl font-bold tracking-[.25em] text-[#ffd15c]">{code}</p></div><p className="mt-7 text-center text-xs tracking-[.18em] text-slate-400">CHOOSE YOUR PART</p><div className="mt-3 grid grid-cols-2 gap-2">{VOICES.map((voice, index) => <div key={voice} className="rounded-xl border p-3" style={{ borderColor: index === part ? COLOURS[index] : '#ffffff16', background: index === part ? `${COLOURS[index]}14` : '#07111d' }}><b style={{ color: COLOURS[index] }}>{voice[0]} <span className="ml-1 text-sm">{voice}</span></b><p className="mt-2 text-xs text-slate-400">{players.filter(item => item.part_index === index).length} players</p></div>)}</div><div className="mt-5"><DifficultyPicker value={difficulty} onChange={setDifficulty} colour={COLOURS[part]} /></div><div className="mt-5"><LatencyRow colour={COLOURS[part]} latencySec={latencySec} onChange={applyLatency} /></div><div className="mt-5"><TransposePicker value={transpose} onChange={setTranspose} colour={COLOURS[part]} hasBackingTrack={hasBackingTrack} /></div><div className="mt-5"><WarmUpToggle value={warmUp} onChange={setWarmUp} colour={COLOURS[part]} /></div><div className="mt-5 rounded-2xl border border-white/10 bg-black/20 p-4"><p className="text-xs text-slate-400">MIC CHECK</p><div className="mt-2 h-2 overflow-hidden rounded-full bg-white/10"><span className="block h-full rounded-full bg-emerald-400 transition-[width] duration-75" style={{ width: `${Math.min(100, Math.round(level * 400))}%` }} /></div><p className="mt-1 text-[10px] text-slate-500">{level > 0.002 ? 'Sound is reaching the app — sing and watch the bar move.' : 'Say something. If the bar stays flat, no audio is arriving.'}</p><div className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1 border-t border-white/10 pt-2 font-mono text-[10px] text-slate-500"><span>note</span><b className={diag.hz > 0 ? 'text-emerald-300' : 'text-slate-600'}>{diag.hz > 0 ? PitchEngine.toNoteName(diag.hz) + '  ' + diag.hz.toFixed(1) + ' Hz' : '—'}</b><span>level</span><b className={diag.level > 0.002 ? 'text-emerald-300' : 'text-slate-600'}>{diag.level.toFixed(4)}</b><span>confidence</span><b>{diag.confidence.toFixed(2)}</b><span>sample rate</span><b>{diag.rate ? (diag.rate / 1000).toFixed(1) + ' kHz' : '—'}</b><span>dc offset</span><b className={Math.abs(diag.dc) > 0.05 ? 'text-rose-300' : 'text-slate-600'}>{diag.dc.toFixed(3)}{Math.abs(diag.dc) > 0.05 ? ' CORRECTED' : ''}</b><span>capture</span><b>{diag.capture}</b><span>build</span><b>{ENGINE_BUILD}</b><span>audio</span><b className={audioBlocked ? 'text-amber-300' : 'text-emerald-300'}>{audioBlocked ? 'PAUSED by browser' : mic === 'ready' ? 'running' : 'not started'}</b><span>window</span><b>{micEnv.standalone ? 'installed app' : 'browser tab'}</b><span>permission</span><b className={micEnv.permission === 'granted' ? 'text-emerald-300' : micEnv.permission === 'denied' ? 'text-rose-300' : 'text-slate-500'}>{micEnv.permission}</b></div><MicHelp reason={micReason} env={micEnv} /><MicReportButton getEngine={getEngine} context="phone-lobby" /><button onClick={testMic} className={`mt-2 text-sm ${mic === 'blocked' ? 'text-rose-300' : 'text-emerald-300'}`}>● {mic === 'ready' ? 'Microphone detected' : mic === 'blocked' ? 'Microphone blocked — tap to retry' : mic === 'checking' ? 'Checking…' : 'Test microphone'}</button></div>{audioBlocked && <button onClick={onResume} className="mt-3 w-full rounded-xl border border-amber-400/40 bg-amber-950/40 px-3 py-2 text-xs text-amber-100">This browser has paused audio input — tap to switch it back on.</button>}{error && <p className="mt-3 rounded-xl border border-rose-400/30 bg-rose-950/40 px-3 py-2 text-xs text-rose-200">{error}</p>}<button onClick={ready} className="vh-start-button mt-5 w-full">✓ {player?.ready_at ? 'I’M READY' : 'TAP WHEN READY'}</button><p className="mt-2 text-center text-[10px] text-slate-500">Tapping ready asks for the microphone now, so the permission prompt cannot land on the downbeat.</p></PhoneShell>; }
function PhoneCountdown({ song, part, phase, mic }: { song: Song; part: number; phase: string; mic: string }) { const number = Number(phase.match(/(\d+)/)?.[1] ?? 0); return <PhoneShell><p className="mt-6 text-center text-sm">{song.title}</p><p className="mt-2 text-center text-xs text-slate-400">YOUR PART</p><h1 className="text-center text-2xl font-black" style={{ color: COLOURS[part] }}>{VOICES[part].toUpperCase()} {VOICES[part][0]}</h1><div className="mx-auto mt-8 grid h-44 w-44 place-items-center rounded-full border-2 border-fuchsia-400 bg-fuchsia-400/[.08] shadow-[0_0_30px_#ec4899]"><div className="text-center"><p className="text-6xl font-black text-fuchsia-300">{number || '•'}</p><p className="mt-1 text-xs tracking-[.2em] text-fuchsia-200">GET READY</p></div></div><div className="mt-6 text-center"><p className="text-2xl text-fuchsia-300">◉</p><p className="mt-1 text-xs text-emerald-300">{mic === 'ready' ? 'READY' : 'MIC CHECK'}</p></div><p className="mt-7 text-center text-sm text-slate-300">◉ Eyes on your part <span className="mx-2 text-slate-600">|</span> ≋ Breathe in</p></PhoneShell>; }
...
function PhoneLive({ song, notes, transpose, warmUp, guide, part, elapsed, pitch, score, hits, sections, mic, fullBoard, setFullBoard, trail, guideAudio, setGuideAudio, getElapsed, getPitch, level }: { song: Song; notes: SongNote[]; transpose: number; warmUp: boolean; guide: boolean; part: number; elapsed: number; pitch: number; score: number; hits: Record<string, boolean>; sections: SectionScore[]; mic: string; fullBoard: boolean; setFullBoard: (value: boolean) => void; trail: TrailSample[]; guideAudio: boolean; setGuideAudio: (on: boolean) => void; getElapsed: () => number; getPitch: () => number; level: number }) {
  ...
  return <PhoneShell fill><div className="mt-3 flex shrink-0 items-center justify-between"><div><p className="text-xs text-slate-400">{song.title}</p><b style={{ color: COLOURS[part] }}>{VOICES[part].toUpperCase()} TEAM</b><div className="mt-1 flex flex-wrap gap-1"><WarmUpBadge active={warmUp} /><TransposeBadge semitones={transpose} colour={COLOURS[part]} /></div></div><div className="text-right"><p className="text-3xl font-black text-fuchsia-300">{score.toLocaleString()}</p><p className="text-[10px] tracking-[.16em] text-slate-500">PERSONAL SCORE</p></div></div><div className="mt-3 shrink-0"><KaraokeLyrics song={song} notes={notes} partIndex={lanePart} elapsed={elapsed} compact /></div><div className="mt-3 min-h-[120px] flex-1"><CanvasLane partIndex={lanePart} partName={guide ? 'Melody guide' : VOICES[part]} colour={guide ? '#ff60bc' : COLOURS[part]} getPosition={getElapsed} notes={notes} getPitchHz={getPitch} getLevel={() => level} hitNotes={hits} lookAheadSeconds={4} showLyrics={false} trail={trail} height={210} fill /></div><section className="vh-panel mt-3 shrink-0 p-3"><div className="flex items-center justify-between"><div><p className="text-[10px] uppercase tracking-wider text-slate-500">You sang</p><b className="text-3xl text-cyan-200">{feedback.detected}</b><p className={`mt-0.5 text-[10px] font-bold ${pitch > 0 ? 'text-emerald-300' : level > 0.01 ? 'text-amber-300' : 'text-slate-600'}`}>{pitch > 0 ? '● hearing you' : level > 0.01 ? '● sound, no note yet' : mic === 'ready' ? '○ no sound reaching the app' : '○ mic not ready'}</p></div><div className="px-2 text-center"><p className={`text-sm font-black ${feedback.state === 'correct' ? 'text-emerald-300' : feedback.state === 'high' || feedback.state === 'low' || feedback.state === 'octave' ? 'text-amber-300' : 'text-slate-400'}`}>{feedback.label}</p><small className="block text-[10px] text-slate-500">{feedback.difference}</small></div><div className="text-right"><p className="text-[10px] uppercase tracking-wider text-slate-500">Target</p><b className="text-3xl text-white">{feedback.target}</b></div></div></section><div className="mt-3 grid shrink-0 grid-cols-2 gap-3"><div className="vh-panel p-3"><p className="text-xs text-slate-400">TEAM ACCURACY</p><b className="text-xl">{Math.round(team?.accuracy ?? 0)}%</b></div><div className="vh-panel p-3"><p className="text-xs text-slate-400">MIC</p><b className="text-xl text-emerald-300">{mic === 'ready' ? 'READY' : 'CHECK'}</b></div></div><button onClick={() => setGuideAudio(!guideAudio)} className={`vh-outline-button mt-2 w-full shrink-0 ${guideAudio ? 'border-emerald-300/40 text-emerald-100' : 'text-slate-400'}`}>{guideAudio ? '♪ Hearing my part — tap to mute' : '♪ Play my part in my ear'}</button>{!guide && <button onClick={() => setFullBoard(!fullBoard)} className="vh-outline-button mt-2 w-full shrink-0">{fullBoard ? 'Return to my part' : 'Show full choir board'}</button>}{fullBoard && <div className="mt-3 space-y-2">{VOICES.map((voice, index) => <CanvasLane key={voice} partIndex={index} partName={voice} colour={COLOURS[index]} getPosition={getElapsed} notes={notes} getPitchHz={index === part ? getPitch : undefined} hitNotes={hits} lookAheadSeconds={4} height={96} showLyrics={false} />)}</div>}</PhoneShell>;
}
