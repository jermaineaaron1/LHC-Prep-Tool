...
function PhoneGame() {
  ...
  return <><PhoneLive song={song!} notes={notes} transpose={transpose} warmUp={warmUp} guide={isGuideMelody(notes)} part={partIndex} elapsed={timeline.songElapsed} pitch={pitch} score={score} hits={hits} sections={sections} mic={mic} fullBoard={fullBoard} setFullBoard={setFullBoard} trail={trailRef.current} guideAudio={guideAudio} setGuideAudio={setGuideAudio} />{preRoll && <CountInOverlay phase={timeline.phase} />}</>;
}
...
function PhoneLive({ song, notes, transpose, warmUp, guide, part, elapsed, pitch, score, hits, sections, mic, fullBoard, setFullBoard, trail, guideAudio, setGuideAudio }: { song: Song; notes: SongNote[]; transpose: number; warmUp: boolean; guide: boolean; part: number; elapsed: number; pitch: number; score: number; hits: Record<string, boolean>; sections: SectionScore[]; mic: string; fullBoard: boolean; setFullBoard: (value: boolean) => void; trail: TrailSample[]; guideAudio: boolean; setGuideAudio: (on: boolean) => void }) {
