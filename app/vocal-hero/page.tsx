...
export default function VocalHeroHostPage() {
  ...
  useEffect(() => {
    ...
    const scorer = new ScoreEngine({
      ...
      onNoteResult: result => setSoloHits(current => ({ ...current, [result.noteId]: result.points > 0 })),
    });
  ...
  async function chooseSong(next: Song) {
    try {
      ...
      setSoloPart(null); setSoloPlayer(null); setSoloMic('unknown'); setSoloPitch(0); setSoloScore(0); setSoloHits({}); setSoloFullBoard(false);
  ...
  const stage = session?.status === 'playing' && song
      ...
      ? soloPlayer && soloPart !== null
        ? <SoloLiveStage song={song} notes={notes} part={soloPart} elapsed={timeline.songElapsed} pitch={soloPitch} score={soloScore} hits={soloHits} sections={sections} mic={soloMic} fullBoard={soloFullBoard} setFullBoard={setSoloFullBoard} />
...
function detectedPitchName(hz: number) {
  if (!hz || hz < 1) return '—';
...
function SoloLiveStage({ song, notes, part, elapsed, pitch, score, hits, sections, mic, fullBoard, setFullBoard }: { song: Song; notes: SongNote[]; part: number; elapsed: number; pitch: number; score: number; hits: Record<string, boolean>; sections: SectionScore[]; mic: string; fullBoard: boolean; setFullBoard: (value: boolean) => void }) {
  ...
  const next = notes.filter(note => (note.part === lanePart || note.part === -1) && note.start > elapsed).sort((a, b) => a.start - b.start)[0];
  const section = sections.find(item => item.part_index === part);
  return <section className="mx-auto max-w-[1350px] px-5 py-6"><div className="grid gap-4 xl:grid-cols-[1fr_280px]"><div><div className="vh-panel flex flex-wrap items-center gap-5 p-4"><SongDetails song={song} /><div className="ml-auto flex items-center gap-5"><div className="text-right"><p className="text-[10px] uppercase tracking-[.18em] text-slate-500">Your voice</p><b className="text-lg" style={{ color: COLOURS[part] }}>{VOICES[part]}</b></div><div className="text-right"><p className="text-3xl font-black text-fuchsia-300">{score.toLocaleString()}</p><p className="text-[9px] uppercase tracking-[.15em] text-slate-500">Personal score</p></div></div></div><div className="mt-4 rounded-2xl border border-white/10 bg-[linear-gradient(135deg,#081326,#120d29)] p-5 text-center shadow-[0_16px_50px_#02061788]"><p className="text-[10px] uppercase tracking-[.22em] text-cyan-300">Sing now</p><p className="mt-2 min-h-12 text-3xl font-black leading-tight text-white sm:text-4xl">{active?.lyric || 'Listen · breathe · prepare'}</p><p className="mt-2 text-sm text-slate-500">{next?.lyric ? `Next: ${next.lyric}` : 'Follow the note rail as it reaches the strike line'}</p></div><div className="mt-4"><SatbLane partIndex={lanePart} partName={guide ? 'Melody guide' : VOICES[part]} colour={guide ? '#ff60bc' : COLOURS[part]} elapsed={elapsed} notes={notes} pitchHz={pitch} hitNotes={hits} lookAheadSeconds={7} showLyrics /></div>{!guide && <button onClick={() => setFullBoard(!fullBoard)} className="vh-outline-button mt-4">{fullBoard ? 'Hide full choir board' : 'Show full choir board'}</button>}{fullBoard && <div className="mt-3 space-y-2">{VOICES.map((voice, index) => <SatbLane key={voice} compact partIndex={index} partName={voice} colour={COLOURS[index]} elapsed={elapsed} notes={notes} pitchHz={index === part ? pitch : undefined} hitNotes={hits} lookAheadSeconds={5} />)}</div>}</div><aside className="vh-panel h-fit p-5"><p className="text-[10px] uppercase tracking-[.2em] text-slate-500">Solo monitor</p><div className="mt-4 grid gap-3 sm:grid-cols-3 xl:grid-cols-1"><div className="rounded-xl border border-white/10 bg-white/[.035] p-4"><p className="text-xs text-slate-400">Detected note</p><b className="mt-1 block text-3xl text-cyan-200">{detectedPitchName(pitch)}</b><small className="text-slate-500">{pitch ? `${Math.round(pitch)} Hz` : 'Sing to begin detection'}</small></div><div className="rounded-xl border border-white/10 bg-white/[.035] p-4"><p className="text-xs text-slate-400">Accuracy</p><b className="mt-1 block text-3xl" style={{ color: COLOURS[part] }}>{Math.round(section?.accuracy ?? 0)}%</b><small className="text-slate-500">Persisted with this session</small></div><div className="rounded-xl border border-emerald-300/20 bg-emerald-300/[.05] p-4"><p className="text-xs text-slate-400">Microphone</p><b className="mt-1 block text-lg text-emerald-300">{mic === 'ready' ? '● READY' : 'CHECK MIC'}</b></div></div></aside></div></section>;
}
