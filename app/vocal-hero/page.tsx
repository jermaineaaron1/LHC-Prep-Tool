...
function LobbyVoice({ name, index, players }: { name: string; index: number; players: SessionPlayer[] }) { const members = players.filter(player => player.part_index === index && !player.is_spectator); const ready = members.filter(player => player.ready_at).length;
  ...
  const present = members.filter(player => isPresent(player.last_seen_at, player.joined_at)).length; return <article className="vh-voice-card" style={{ '--voice': COLOURS[index] } as React.CSSProperties}><div className="flex items-center justify-between"><div className="flex items-center gap-3"><b className="text-4xl" style={{ color: COLOURS[index] }}>{name[0]}</b><div><h2 className="font-bold" style={{ color: COLOURS[index] }}>{name.toUpperCase()}</h2><p className="text-xs text-slate-400">{members.length} players</p></div></div><span className="text-xs" style={{ color: COLOURS[index] }}>Ready {ready}/{members.length}{present < members.length ? ` · ${members.length - present} away` : ''}</span></div><div className="mt-3 h-1 overflow-hidden rounded-full bg-white/10"><span className="block h-full rounded-full" style={{ background: COLOURS[index], width: `${members.length ? ready / members.length * 100 : 0}%` }} /></div><div className="mt-3 space-y-2">{members.slice(0, 7).map(player => { const here = isPresent(player.last_seen_at, player.joined_at); return <div key={player.id} className={`flex items-center gap-2 text-sm ${here ? '' : 'opacity-45'}`}><Avatar name={player.player_name} colour={COLOURS[index]} /><span className="min-w-0 flex-1 truncate">{player.player_name}</span>{!here && <span className="text-[10px] text-slate-500">{lastSeenLabel(player.last_seen_at)}</span>}<span title={here ? 'In the room' : 'Not answering'} className={`h-2 w-2 rounded-full ${!here ? 'bg-slate-700' : player.mic_status === 'ready' ? 'bg-emerald-400' : 'bg-slate-600'}`} /></div>; })}{!members.length && <p className="py-6 text-center text-xs text-slate-500">Waiting for singers</p>}</div></article>; }
...
function SoloLiveStage({ song, notes, transpose, warmUp, part, elapsed, getElapsed, getPitch, getLevel, pitch, score, hits, lastResult, sections, mic, fullBoard, setFullBoard, trail }: { song: Song; notes: SongNote[]; transpose: number; warmUp: boolean; part: number; elapsed: number; getElapsed: () => number; getPitch: () => number; getLevel: () => number; pitch: number; score: number; hits: Record<string, boolean>; lastResult: NoteScoreResult | null; sections: SectionScore[]; mic: string; fullBoard: boolean; setFullBoard: (value: boolean) => void; trail: TrailSample[] }) {
    ...
    <div className="vh-panel p-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="text-right">
          ...
          <p className="text-5xl font-black text-fuchsia-300">{score.toLocaleString()}</p>
        </div>
      </div>
