...
export default function VocalHeroHostPage() {
  ...
  useEffect(() => {
    ...
    const scorer = new ScoreEngine({
      ...
      playerId: soloPlayer.id, sessionId: session.id, difficulty,
    ...
    scorer.start();
  }, [difficulty, session, soloPart, soloPlayer, song, transpose]);
  useEffect(() => {
  ...
  async function startSoloPitchTracking(part: number) {
    ...
    const engine = new PitchEngine({ bufferSize: 2048, confidenceThreshold: .76, smoothing: .22, minHz: PitchEngine.midiToHz(range.low - 3 + shift), maxHz: PitchEngine.midiToHz(range.high + 3 + shift), onPitch: sample => {
  ...
  const stage = session?.status === 'playing' && song
      ...
      ? soloPlayer && soloPart !== null
        ? <SoloLiveStage song={song} notes={soloNotes} transpose={transpose} part={soloPart} elapsed={timeline.songElapsed} pitch={soloPitch} score={soloScore} hits={soloHits} lastResult={soloLastResult} sections={sections} mic={soloMic} fullBoard={soloFullBoard} setFullBoard={setSoloFullBoard} trail={trailRef.current} />
    ...
    : session?.status === 'ended' && song && soloReview && soloPart !== null
      ? <SoloReviewStage song={song} part={soloPart} score={soloScore} review={soloReview} onDone={returnToLibrary} />
    : session && song
      ? <Lobby song={song} session={session} players={players} phoneUrl={phoneUrl} onStart={start} onStartSolo={startSolo} soloStarting={soloStarting} difficulty={difficulty} setDifficulty={setDifficulty} latencySec={latencySec} applyLatency={applyLatency} transpose={transpose} setTranspose={setTranspose} />
...
function Lobby({ song, session, players, phoneUrl, onStart, onStartSolo, soloStarting, difficulty, setDifficulty, latencySec, applyLatency, transpose, setTranspose }: { song: Song; session: GameSession; players: SessionPlayer[]; phoneUrl: string; onStart: () => void; onStartSolo: (part: number) => void; soloStarting: number | null; difficulty: Difficulty; setDifficulty: (next: Difficulty) => void; latencySec: number; applyLatency: (seconds: number) => void; transpose: number; setTranspose: (next: number) => void }) {
...
function SoloLiveStage({ song, notes, transpose, part, elapsed, pitch, score, hits, lastResult, sections, mic, fullBoard, setFullBoard, trail }: { song: Song; notes: SongNote[]; transpose: number; part: number; elapsed: number; pitch: number; score: number; hits: Record<string, boolean>; lastResult: NoteScoreResult | null; sections: SectionScore[]; mic: string; fullBoard: boolean; setFullBoard: (value: boolean) => void; trail: TrailSample[] }) {
...
function SoloReviewStage({ song, part, score, review, onDone }: {
  song: Song; part: number; score: number; review: RoundReview; onDone: () => void;
}) {
