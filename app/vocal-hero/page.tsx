...
function SoloCountdownStage({ song, part, phase, mic }: { song: Song; part: number; phase: string; mic: string }) {
  ...
  return <section className="mx-auto max-w-[1100px] px-3 py-3 sm:px-5 sm:py-7"><div className="vh-panel overflow-hidden p-4 sm:p-6"><div className="flex flex-wrap items-center justify-between gap-4"><SongDetails song={song} /><div className="rounded-xl border px-4 py-3 text-right" style={{ borderColor: `${COLOURS[part]}55`, background: `${COLOURS[part]}0d` }}><p className="text-[10px] uppercase tracking-[.18em] text-slate-500">Solo voice</p><b className="text-xl" style={{ color: COLOURS[part] }}>{VOICES[part]}</b></div></div><div className="mx-auto mt-4 grid min-h-[280px] max-w-xl place-items-center text-center sm:mt-7 sm:min-h-[430px]"><div className="vh-count-ring"><div><p className="text-xs tracking-[.35em]" style={{ color: COLOURS[part] }}>SOLO PRACTICE</p><p className="mt-2 text-7xl font-black leading-none sm:mt-3 sm:text-[10rem] text-transparent [text-shadow:0_0_40px_#ec4899] bg-gradient-to-br from-fuchsia-400 via-violet-400 to-cyan-300 bg-clip-text">{number || '•'}</p><p className="text-lg font-semibold text-fuchsia-200">{phase.includes('Lead') ? 'Listen · breathe · prepare' : 'SONG BEGINS IN'}</p></div></div></div><footer className="flex flex-wrap items-center justify-center gap-4 text-sm"><span className="rounded-full border border-emerald-300/25 bg-emerald-300/[.08] px-3 py-1.5 text-emerald-200">● {mic === 'ready' ? 'Microphone ready' : 'Checking microphone'}</span><span className="text-slate-400">Eyes on your {VOICES[part]} line</span></footer></div></section>;
}
