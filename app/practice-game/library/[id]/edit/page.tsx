...
// ── Constants ──────────────────────────────────────────────────────────────────
const KEY_W   = 68;   // piano strip width  (CSS px)
const NOTE_H  = 16;   // px per semitone row
const RULER_H = 26;   // time ruler height
const MIDI_LO = 36;   // C2
const MIDI_HI = 84;   // C6
const EDGE    = 8;    // resize-grip zone (px)
...
export default function SongEditorPage() {
  ...
  const drawModeR = useRef<'draw'|'select'>('draw');
  ...
  useEffect(()=>{ snapR.current    = snap;     },[snap]);
  ...
  useEffect(()=>{ drawModeR.current = drawMode;
    setCursor(drawMode==='select' ? 'default' : 'crosshair'); },[drawMode]);
  ...
  const draw = useCallback(()=>{
    ...
    // ── Time grid ────────────────────────────────────────────────────────────
    const t0  = Math.floor(sx);
    const t1  = Math.ceil(sx + (W-KEY_W)/z) + 1;
    const bar = z>100?0.5:z>50?1:z>25?2:4;  // bar width in seconds
    for (let t=t0; t<=t1; t+=0.25) {
      const x = noteX(t,sx,z);
      if (x<KEY_W||x>W) continue;
      const isBar   = (t%bar===0)&&bar<=1 || Number.isInteger(t/bar);
      const isBeat  = Number.isInteger(t);
      ctx.strokeStyle = isBar?'rgba(124,58,237,0.2)':isBeat?'rgba(124,58,237,0.12)':'rgba(124,58,237,0.05)';
    ...
    for (let m=MIDI_LO; m<=MIDI_HI; m++) {
      ...
      // White keys: bright white/cream; Black keys: dark charcoal
      ctx.fillStyle = blk ? '#2a2a2a' : '#f0ece8';
      ctx.fillRect(1, y+0.5, blk?KEY_W-22:KEY_W-2, NOTE_H-1);

      // White key right-edge shadow
      if (!blk) {
        ...
        ctx.fillRect(1, y+1, KEY_W-23, 3);
      }

      // Show note name on every natural (white) key; octave labels bold
      if (!blk) {
        const isOct = m%12===0;
        ctx.fillStyle = isOct ? '#7c6aaa' : '#888';
        ctx.font = `${isOct?'bold ':''}8px monospace`;
        ctx.textAlign='right';
    ...
    ctx.fillStyle='rgba(124,58,237,0.5)'; ctx.font='10px monospace';
    for (let t=t0; t<=t1; t++) {
      const x = noteX(t,sx,z);
      if (x<KEY_W||x>W) continue;
      const show = z>80?(t%2===0):z>30?(t%5===0):(t%15===0);
      if (show) {
        ctx.strokeStyle='rgba(124,58,237,0.25)'; ctx.lineWidth=1;
        ctx.beginPath(); ctx.moveTo(x,RULER_H-5); ctx.lineTo(x,RULER_H-1); ctx.stroke();
        ctx.fillText(fmtTime(t), x+2, RULER_H-6);
      }
    }
  ...
  useEffect(()=>{ draw(); },[draw, notes, scrollX, scrollY, zoom, selId, ytTime, duration, vis, snap]);
  ...
  function cursorFor(hit:{zone:string}|null): string {
    if (!hit) return drawModeR.current==='select' ? 'default' : 'crosshair';
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="flex rounded overflow-hidden border border-purple-900/40 flex-shrink-0">
              <button
                ...
                title="Draw mode: click to place notes"
                className={`text-xs px-2.5 py-1 transition-colors flex items-center gap-1 ${drawMode==='draw'?'bg-[#7c3aed] text-white':'bg-[#14142a] text-gray-500 hover:text-gray-300'}`}>
                ✏ Draw
              </button>
              <button
            ...
            {([0,0.0625,0.125,0.25,0.5,1]).map(v=>(
              <button key={v} onClick={()=>setSnap(v)}
                className={`text-xs px-1.5 py-0.5 rounded border transition-colors ${snap===v?'bg-purple-900/60 border-purple-500 text-purple-200':'border-purple-900/30 text-gray-500 hover:text-white'}`}>
                {v===0?'Off':v===0.0625?'1/16':v===0.125?'1/8':v===0.25?'1/4':v===0.5?'1/2':'1'}
              </button>
            ))}
          </div>
          {/* Part selector */}
      ...
      <div className="flex-none bg-[#14142a] border-t border-purple-900/30 px-3 py-2 flex items-center gap-2 flex-wrap min-h-[48px]">
        {selNote ? (
          ...
          <p className="text-xs text-gray-700">
            {drawMode==='select'
              ? <><b className="text-[#22d3ee]">↖ Select mode</b> &nbsp;·&nbsp; <b className="text-gray-500">Click</b> = select note &nbsp;·&nbsp; <b className="text-gray-500">Drag</b> = box select &nbsp;·&nbsp; <b className="text-gray-500">Delete</b> = remove &nbsp;·&nbsp; <b className="text-gray-500">Esc</b> = deselect</>
              : <><b className="text-purple-400">✏ Draw mode</b> &nbsp;·&nbsp; <b className="text-gray-500">Click</b> = place note &nbsp;·&nbsp; <b className="text-gray-500">Drag</b> = resize &nbsp;·&nbsp; <b className="text-gray-500">Ctrl+Drag</b> = box select &nbsp;·&nbsp; <b className="text-gray-500">Scroll</b> = pan timeline &nbsp;·&nbsp; <b className="text-gray-500">Shift+Scroll</b> = horizontal</>
            }
          </p>
        )}
      </div>
